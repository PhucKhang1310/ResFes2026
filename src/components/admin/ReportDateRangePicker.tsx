  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        ...
        className="flex min-h-14 min-w-64 cursor-pointer items-center gap-3 rounded border border-amber-50/15 bg-black px-3 py-2 text-left transition hover:border-[#ff6a1f]"
        onClick={() => setIsOpen((current) => !current)}
      >
        <FaCalendarDays className="shrink-0 text-[#ff6a1f]" aria-hidden="true" />
        <span className="grid flex-1 grid-cols-[1fr_auto_1fr] items-center gap-2 text-sm">
          <span>
            <span className="block text-xs font-semibold text-amber-50/45">From</span>
            <span className="text-amber-50">{displayDate(from)}</span>
          </span>
          <span className="text-amber-50/30">to</span>
          <span>
            <span className="block text-xs font-semibold text-amber-50/45">To</span>
      ...
      {isOpen ? (
        ...
        >
          <div className="mb-1 flex items-center justify-between">
            <span className="text-sm font-semibold text-amber-50">Report dates</span>
            <button
              type="button"
